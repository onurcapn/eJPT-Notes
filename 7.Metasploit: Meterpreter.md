...
```
* **Sömürü:** `/usr/local/bin/custom_backup` gibi özel veya zafiyet barındıran bir SUID dosyası varsa, bu dosyanın sistem komutlarını nasıl çalıştırdığını inceleyerek (örn: path injection veya buffer overflow) root yetkisi kazanabiliriz.
