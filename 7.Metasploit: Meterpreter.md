...
* **`getpid`**: Meterpreter payload'umuzun o an hangi süreç (process) ID'si altında çalıştığını gösterir.

* **`ipconfig` / `ifconfig`**: Hedefin ağ arayüzlerini listeler.
...
1. **Çalışan süreçleri listele (`ps`):**
   ...
   meterpreter > ps
   ```
   * Windows için en kararlı ve güvenli süreçler kullanıcı yetkisindeyken `explorer.exe` (Masaüstü süreci), sistem yetkisindeyken ise `services.exe` veya `winlogon.exe`'dir.
