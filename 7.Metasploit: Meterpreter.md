...
Çalıştırıldığında dosyayı oluşturan kullanıcının (genellikle root) yetkileriyle çalışan SUID bitli dosyaları tararız.
```bash
...
```
* **Sömürü:** `/usr/local/bin/custom_backup` gibi özel veya zafiyet barındıran bir SUID dosyası varsa, bunun üzerinden **PATH Injection (Yol Zehirlemesi)** tekniği ile root yetkisi kazanabiliriz.

  > [!NOTE]
  > **SUID Mantığı ve PATH Injection Sömürü Adımları:**
  > 
  > **SUID Nedir?**
  > SUID biti ayarlanmış bir dosya çalıştırıldığında, onu çalıştıran kullanıcının yetkileri yerine **dosya sahibinin (genellikle root)** yetkileriyle çalışır. Yani biz standart kullanıcıyken bile bu dosya root olarak yürütülür.
  > 
  > **PATH Injection (Yol Zehirlemesi) Nasıl Yapılır?**
  > Özel yazılmış SUID dosyaları (örn: `/usr/local/bin/custom_backup`), kendi kodları içerisinde bazen standart sistem araçlarını (örn: `tar`, `cat`, `gzip`) tam yollarını (örn: `/bin/tar` yerine sadece `tar` yazarak) belirtmeden çalıştırır. Sistem bu komutu bulmak için `$PATH` çevre değişkenindeki dizinleri sırayla arar. Bunu şöyle sömürürüz:
  > 
  > 1. **Analiz:** Dosyanın içindeki okunabilir yazıları inceleyerek hangi komutları çalıştırdığını buluruz:
  >    ```bash
  >    $ strings /usr/local/bin/custom_backup
  >    # Çıktıda "tar -czf backup.tar.gz ..." gibi bir satır gördüğümüzü varsayalım.
  >    ```
  > 2. **Zehirleme (Poisoning):** `/tmp` gibi yazabildiğimiz bir dizine gidip, SUID programının çağırdığı komutla (`tar`) aynı isimde sahte bir dosya oluştururuz. Bu dosyanın içine bize shell vermesini söyleyen kodu yazarız:
  >    ```bash
  >    $ echo '/bin/sh' > /tmp/tar
  >    $ chmod +x /tmp/tar
  >    ```
  > 3. **PATH Değişkenini Güncelleme:** Sistemimizin `$PATH` arama sırasını, önce bizim sahte `tar` dosyamızın bulunduğu `/tmp` dizinine bakacak şekilde değiştiririz:
  >    ```bash
  >    $ export PATH=/tmp:$PATH
  >    ```
  > 4. **Çalıştırma:** SUID programını çalıştırırız:
  >    ```bash
  >    $ /usr/local/bin/custom_backup
  >    ```
  > * **Sonuç:** Program root yetkisinde çalışırken `tar` komutunu çalıştırmak ister. `$PATH` değişkeninde en başta `/tmp` olduğu için sistem bizim sahte `/tmp/tar` dosyamızı root yetkileriyle çalıştırır ve bize **root yetkili bir shell** açılır!

#### C. Local Exploit Suggester Kullanımı
Metasploit'in `local_exploit_suggester` modülü Linux kernel (çekirdek) zafiyetlerini önermek için de kullanılabilir.
```bash
...
```
* Önerilen Linux yerel exploit modülünü (örn: `exploit/linux/local/dirtycow` vb.) kullanarak root oturumu alabilirsiniz.
