...
Sisteme sıradan bir kullanıcı olarak sızdıysak (örn: Windows için `getuid` sonucu `WIN-VICTIM\student` veya Linux için `www-data` ise), en yetkili seviyeye çıkmak için şu teknikleri deneriz:
...
### D. Linux Yetki Yükseltme Yöntemleri (Linux Privilege Escalation)
...
#### 1. Sudo Yetkilerinin Analizi (`sudo -l`)
...
#### 2. SUID (Set Owner User ID) Yetkili Dosyaların Tespiti
...
#### 3. Linux Local Exploit Suggester Kullanımı
Windows'ta olduğu gibi, `local_exploit_suggester` modülü Linux kernel (çekirdek) zafiyetlerini önermek için de kullanılabilir.
```bash
...
```
* **Beklenen Çıktı:** Sistem çekirdeğinin (kernel) sürümüne ve yüklü yamalara göre zafiyet listesi dökülür. Önerilen Linux yerel exploit modülünü (örn: `exploit/linux/local/dirtycow` vb.) kullanarak root oturumu alabilirsiniz.
