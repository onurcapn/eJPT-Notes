...
> 2. Parametreleri listeleyin: `show options`
> 3. **Eğer `LHOST` mevcutsa:** `set LHOST tun0` komutuyla kendi IP'nizi tanımlayın, payload'u yapılandırın ve `exploit` edin (Ters Bağlantı).
> 4. **Eğer `LHOST` mevcut değilse:** Sadece hedef IP'yi (`RHOSTS`) tanımlayın ve doğrudan `exploit` / `run` edin (Düz Bağlantı veya Yardımcı Tarayıcı).
