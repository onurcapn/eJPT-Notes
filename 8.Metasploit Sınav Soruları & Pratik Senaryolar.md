...
> ---
> 
> **💡 Pratik Sınav Kuralı (LHOST / Payload Akışı):**
> 1. Modülü seçin: `use exploit/...`
> 2. Parametreleri listelemek için şu komutu çalıştırın: `show options`
> 3. **Eğer `show options` çıktısındaki tabloda (sol sütundaki parametre isimleri arasında) `LHOST` değişkeni yer alıyorsa:** Bu durum, kurban makinenin sizin Kali sisteminize geri bağlanacağı bir **Ters Bağlantı (Reverse Shell)** senaryosudur. `set LHOST tun0` komutuyla kendi IP'nizi tanımlayın, ardından `exploit` edin.
> 4. **Eğer `show options` çıktısındaki tabloda `LHOST` adında bir parametre yer almıyorsa (yoksa):** Bu durum, kurbanın port açıp sizin ona bağlanacağınız bir **Düz Bağlantı (Bind Shell/Backdoor)** senaryosudur veya bir yardımcı tarayıcı (auxiliary) modülüdür. Sadece hedef IP'yi (`RHOSTS`) tanımlayın ve doğrudan `exploit` / `run` edin.
